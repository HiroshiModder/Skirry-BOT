...
class WAConnection extends _5_User_1.WAConnection {
    /**
     * Envie uma mensagem para o ID fornecido (pode ser em grupo, individual ou broadcast)
     * @param id o id a ser enviado para
     * @param mensagem a mensagem pode ser um buffer, string simples, mensagem de localização, texto estendido 
    ...
    /**
     * Envie uma mensagem de lista
     * @param id o id para enviar para
     * @botão param o botão opcional de texto, título e botão de descrição
     * @param rows as linhas da mensagem da lista de seções
     */
    ...
    /** Prepara uma mensagem para enviar via sendWAMessage () */
    ...
    /**
     * Alterna mensagens de desaparecimento para o chat em questão
     *
     * @param jid o chat para alternar
     * @param ephemeralExpiration 0 para desativar, insira qualquer número positivo para ativar o desaparecimento de mensagens pela duração especificada;
     * Para o padrão, consulte WA_DEFAULT_EPHEMERAL
     */
        ...
        switch (type) {
            ...
            case Constants_1.MessageType.extendedText:
                ...
                if ('text' in message) {
                    ...
                    throw new Constants_1.BaileysError('a mensagem precisa ser uma string ou objeto com propriedade \'text\'', message);
                }
                break;
    ...
    async prepareMessageMedia(media, mediaType, options = {}) {
        ...
        // URL seguro Base64 codifica o hash SHA256 do corpo
        ...
        if (requiresDurationComputation) {
            try {
                ...
                this.logger.debug({ error }, 'falha ao obter a duração do áudio: ' + error.message);
            }
        }
        // envie uma consulta JSON para obter o token de url e autenticação para fazer upload de nossa mídia
        let json = await this.refreshMediaConn(options.forceNewMediaOptions);
        let mediaUrl;
        for (let host of json.hosts) {
            const auth = encodeURIComponent(json.auth); // o token de autenticação
            ...
            try {
                ...
                this.logger.error(`Error in uploading to ${host.hostname} (${error}) ${isLast ? '' : ', tentando novamente...'}`);
            }
        }
        if (!mediaUrl)
            throw new Error('O upload de mídia falhou em todos os hosts');
        ...
        return Constants_1.WAMessageProto.Message.fromObject(message); // como WAMessageContent
    }
    /** prepara uma WAMessage para enviar a partir do conteúdo e das opções fornecidas */
    prepareMessageFromContent(id, message, options) {
        if (!options.timestamp)
            //options.timestamp = new Date(); // definir carimbo de data / hora para agora
        if (typeof options.sendEphemeral === 'undefined')
            options.sendEphemeral = 'chat';
        if (options.viewOnce)
            message = { viewOnceMessage: { message } };
        // evitar um bug irritante (WA não aceita o envio de mensagens com '@ c.us')
        ...
        if (quoted) {
            ...
            // se um participante é citado, então deve ser um grupo
            // portanto, remoteJid do grupo também deve ser inserido
        ...
        if (
        // se quisermos enviar uma mensagem de desaparecimento
        (((options === null || options === void 0 ? void 0 : options.sendEphemeral) === 'chat' && (chat === null || chat === void 0 ? void 0 : chat.ephemeral)) ||
            ...
            // e não é uma mensagem de protocolo - excluir, alternar para desaparecer mensagem
            key !== 'protocolMessage' &&
            // já não foi convertido para mensagem que está desaparecendo
